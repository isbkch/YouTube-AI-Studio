...
struct ReviewView: View {
 ...
 var body: some View { HSplitView {
  ScrollView { VStack(alignment: .leading, spacing: 20) {
   ...
   if let plan = p.plan { Text("Jump to scene").font(.headline); LazyVGrid(columns: [GridItem(.adaptive(minimum: 105))]) { ForEach(plan.scenes) { s in Button { sceneID = s.id; player?.seek(to: CMTime(seconds: Double(s.startFrame) / Double(plan.frameRate), preferredTimescale: 600)) } label: { VStack(spacing: 4) { Text(s.id); Text(timestamp(Double(s.startFrame) / Double(plan.frameRate))).foregroundStyle(.secondary) }.font(.caption).frame(maxWidth: .infinity).padding(10) }.buttonStyle(.bordered) } } }
  }.padding(25) }.frame(minWidth: 500)
  ScrollView { VStack(alignment: .leading, spacing: 20) {
   ...
   Divider()
   ForEach(p.revisions.reversed()) { r in VStack(alignment: .leading, spacing: 12) {
    Text(r.patch.originatingRequest).font(.headline)
    Text(r.patch.rationale).font(.caption).foregroundStyle(.secondary)
    Text("v\(r.patch.previousVersion) → v\(r.patch.resultingVersion) · \(r.patch.affectedScenes.joined(separator: ", "))").font(.system(size: 10, design: .monospaced)).foregroundStyle(Color.studioAccent)
    ForEach(r.patch.affectedScenes, id: \.self) { id in if let s = p.plan?.scenes.first(where: { $0.id == id }) { Text("Current \(id): \(s.visual.graphic?.template ?? "Presenter") — \(s.visual.description)").font(.caption).foregroundStyle(.secondary) } }
    DisclosureGroup("Proposed operations") { ForEach(Array(r.patch.operations.enumerated()), id: \.offset) { _, op in Text(op.pretty).font(.system(size: 10, design: .monospaced)).textSelection(.enabled).frame(maxWidth: .infinity, alignment: .leading) } }
    if r.status == "PROPOSED" { HStack { Button("Apply Proposal") { Task { await m.perform("revision.decide", label: "Revision applied", params: ["patchId": r.id, "apply": true]) } }.disabled(m.busy || r.patch.previousVersion != p.plan?.version); Button("Reject") { Task { await m.perform("revision.decide", label: "Revision rejected", params: ["patchId": r.id, "apply": false]) } }.disabled(m.busy) } } else { Text(r.status.capitalized).font(.caption).foregroundStyle(.secondary) }
