...
- **Milestone 5 — packaging and publishing (2026-09-16).** The last gate is live. After the final render, a **Packaging agent** proposes ranked title candidates (with a recommendation), 2–4 thumbnail concepts (short emotional headline + art direction), the full description — opening, body, the exact chapter timestamps YouTube parses, and the research sources — plus metadata (tags under the 500-char budget, category, `private` visibility, `madeForKids: false`) as one versioned document (`packaging/packaging-v{N}.json`). Chapter timestamps are derived from the rendered plan and the agent may only re-title them, never re-time them; validation enforces YouTube's rules (first chapter 0:00, ≥10s apart, inside the render) and description length. **Packaging approval** binds to the document hash; `wts publish` re-verifies the artifact, uploads the final render through the local `youtubeuploader` CLI (`youtube.ts`: argument arrays without a shell, `WTS_YOUTUBEUPLOADER_PATH`/`WTS_YOUTUBE_ARGS`, no credentials handled by the runtime), parses the video ID and records `publication` exactly once (status `PUBLISHED`, previously unreachable). CLI: `wts packaging | packaging approve | publish`; IPC: `packaging.run/get/approve`, `publish.run`; native app: Packaging & publishing card in Review, live gate 03 in Overview, doctor reports the YouTube CLI. Mock is deterministic; the stub-CLI test proves the argv shape (no shell injection), meta JSON, hash tampering rejection and one-shot semantics. 4 Milestone-5 unit tests plus a provider-transport test (55 tests total).
