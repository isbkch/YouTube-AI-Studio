...
test("jobs retry recoverable failures, respect dependencies and concurrency", async () => {
    ...
    [
      ...tasks,
      {
        ...
        run: async () => {
          ...
          if (++attempts === 1)
            throw new StudioError("EXTERNAL_TOOL", "Transient", "Retry", true);
        },
      },
    ],
    "p",
  ...
  const jobs = await graph.run();
  assert.equal(max, 2);
  assert.equal(attempts, 2);
  assert.equal(jobs[2].retryCount, 1);
...
test("failed dependency blocks assembly without rebuilding successful jobs", async () => {
    ...
    [
      {
        ...
        dependencies: [],
        run: async () => {
          throw new StudioError("INVALID_INPUT", "Invalid template");
        },
      },
      {
        id: "b",
  ...
  assert.equal(graph.jobs[1].status, "BLOCKED");
});
