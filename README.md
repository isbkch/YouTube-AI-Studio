...
**Autonomy — the Producer.** Each project is `supervised` (default) or `autonomous`. On autonomous projects the deterministic Producer (`packages/orchestrator/src/producer.ts`, no model calls, zero cost) auto-approves the machine gates — storyboard and rough cut — with a total evidence trail: reviews and findings persist in `Project.producerReviews`, approvals record `approvedBy: "producer"`, and benign warnings (pacing pauses, spare takes) approve with the evidence recorded while real deviations (omission ratio over 25%, duration outside 0.4×–1.6× of target, hot peaks, flagged scenes) escalate back to you instead of shipping. Mechanical escalations repair themselves first, within a budget of two per advance: over/under-target cuts re-direct at a tighter or looser pacing, and QA-flagged scenes get their visual treatment disabled (footage, audio and duration untouched). Semantic findings — missing spoken words — always come to you. `wts producer --all` runs one Producer pass over every autonomous project, self-healing crash-stranded builds first. The script gate (your words) and the publication gate (one-shot external upload) are never auto-approved in any mode.

**Pickups instead of reshoots.** When sentences of the approved script aren't spoken in any take, `wts rerecord <project>` prints the pickup list: each missing sentence with its recorded omission reason and the surrounding sentences for delivery context. Record just those lines as one short take, import and transcribe it, and re-run plan generation — the alignment matches across every take and splices the pickup in, no reshoot needed.
