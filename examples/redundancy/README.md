...
Generated outputs live in `.demo`, not Git. `.demo/demo-result.json` identifies the newest project, MP4 preview and Resolve export. Each project contains timeline JSON, FCPXML, OTIO, QA, plan/patch history, derived media and provenance stored in SQLite. Use `pnpm macos:demo` to inspect them in the native app. Demo harness approvals are test actions; the resulting rough cut remains awaiting human approval.
