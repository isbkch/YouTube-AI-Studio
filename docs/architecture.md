...
`retakes.ts` derives a non-destructive transcript review: adjacent repeated sentences within one recording prefer the last complete delivery when separated by at most ten seconds. Matching tolerates article/filler differences but preserves negation, numbers and content words. Short rhetorical beats and sentences repeated in the approved script are excluded from automatic collapse. Real word timings allow sentence boundaries within/across segments; without them, whole segments are the smallest safe unit. Original transcript text, IDs and timestamps remain authoritative. Snapshots add `retakeReview` for the native **Show all attempts** toggle. Alignment (`smith-waterman-v5-last-retake`) excludes discarded ranges; matching, padding, bridging, grouping and tightening cannot reintroduce them. Older alignments are invalidated by their algorithm identity; existing plans and approvals remain bound to their original selections.
...
When sentence alignment is available, AI storyboard generation starts from the validated deterministic A-roll cut. The Director returns scene treatments (graphics, framing, chapter titles, music intensity and rationale) keyed by existing scene IDs; it cannot rewrite footage, timing or narration. Unlisted scenes remain presenter footage. Transcript references are derived from the selected recording and frame range, and the resulting plan still passes source validation. Without alignment, the full-plan fallback remains subject to the same validation. Returned Director candidates and their usage are saved in the project's `logs/` before validation, including rejected responses. Shooting-script quote markers and production directions are excluded from narration and chapter headings. Storyboard presenter thumbnails sample each scene's own selected recording and source time; graphic cards remain placeholders until rendering.
...
Packaging approval binds to the exact saved document and, when selected, a thumbnail descriptor (packaging version/hash, slot, immutable revision, managed path and output hash). Publishing revalidates both before the local YouTube CLI starts. A selected revision remains pinned while new drafts or images are explored; only changing the selected revision invalidates its approval. Regenerating packaging clears the selection and approval. The adapter records a created video ID immediately, including when the subsequent thumbnail upload fails, so retrying cannot duplicate that video. Published projects are read-only for thumbnail edits and retain export access.

`thumbnails.ts` owns the two-slot workflow under the existing project lock. Per-slot jobs persist verified backgrounds before composition, charge provider usage once, retain successful revisions and recover interrupted states. The checked-in `templates/remotion/thumbnail.tsx` still composition uses a separate Remotion bundle/cache identity so thumbnail changes do not invalidate video graphics. Exact text is fitted locally into two lines, with overflow rejected rather than truncated. Images are hash-checked and decoded before approval, export and upload; exports copy exact bytes into a fresh folder. Both CLI and private IPC call these same domain methods. A/B means comparison and export; YouTube Studio handles live testing.
