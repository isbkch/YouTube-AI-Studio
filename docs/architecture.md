...
Creator defaults and explicit preferences are editable local data. A project snapshots its profile to make past generation decisions reproducible. New application defaults do not silently change active projects. Analytics observations are separate from preference records and carry no inferred causation or automatic preference updates.
